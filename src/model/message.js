...
export class Message extends Model {
    ...
    static sendAudio(chatId, from, file, metadata, photo){
        ...
        return Base64.toDataURL(file).then(audioDataURL => {
    ...
    static send(chatId, from, type, content, data = {}) {
        ...
        if (!chatId) {
            ...
            return Promise.reject(new Error('chatId é obrigatório'));
        }
    ...
    static getRef(chatId) {
        if (!chatId) {
            throw new Error('chatId não informado em Message.getRef()');
        }
